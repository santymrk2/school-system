...
## 2. Roles y Permisos
- **Dirección**: acceso completo. Puede crear actas, editar registros existentes, cerrar actas, asignar direcciones firmantes, marcarlas como firmadas, eliminar registros y exportar listados.
- **Secretaría**: mismas capacidades que Dirección. Puede gestionar todo el ciclo de vida del acta y exportar información.
- **Administración**: puede ingresar a la sección, registrar nuevas actas y generar exportaciones, pero no editar, cerrar, firmar ni eliminar actas existentes.
- **Docentes/Preceptores (Teacher/Alternate)**: acceso para crear actas de sus secciones vigentes. No pueden editar ni eliminar registros previos, ni exportar información.
- **Familias/Estudiantes**: visualización únicamente de las actas asociadas a sus hijos o a sí mismos mediante una vista simplificada.
- **Otros perfiles**: reciben un mensaje de acceso denegado y no pueden interactuar con la sección.
...
- **Resultado esperado**: Se muestra el encabezado “Actas de Accidentes” y, según su rol, aparecen los filtros, estadísticas y listado correspondiente.
...
**Ubicación**: Panel principal de la sección, debajo del encabezado “Actas de Accidentes”.
...
- **Validaciones**: Las cifras se actualizan en función del total filtrado (firmadas vs. pendientes).
...
- **Validaciones**: Las etiquetas de estado cambian de color según el estado (Borrador, Cerrada, Firmada) y se muestran badges adicionales para informante/firmante cuando corresponde.
...
- ❌ **Error**: “No hay actas para el criterio seleccionado.” cuando los filtros no devuelven resultados.
- ⚠️ **Advertencia**: Ninguna específica en esta vista.

#### Casos Especiales:
- Si el perfil pertenece a familia/estudiante se carga una vista diferente (ver funcionalidad 4.9).
...
**Ubicación**: Barra de filtros inmediatamente debajo del encabezado de la sección.
...
- **Validaciones**: La lista muestra únicamente alumnos presentes en las actas cargadas.
...
- **Validaciones**: El sistema distingue automáticamente estados y excluye combinaciones incompatibles (por ejemplo, “Pendientes” excluye firmadas).
...
- **Validaciones**: Si se establece una fecha inicial posterior a la final (o viceversa) el sistema corrige el rango para mantener coherencia.
...
- **Validaciones**: El sistema realiza coincidencia parcial sobre múltiples campos (alumno, DNI, familiar, sección, descripción, firmante, lugar y acciones).
...
#### Mensajes del Sistema:
- ✅ **Éxito**: No aplica.
- ❌ **Error**: “No hay actas para el criterio seleccionado.” cuando la combinación no devuelve resultados.
- ⚠️ **Advertencia**: Ninguna adicional.

#### Casos Especiales:
- Los filtros afectan también la exportación CSV (ver funcionalidad 4.3).

### 4.3 Exportar actas a CSV
**Descripción**: Genera un archivo CSV con los registros filtrados para análisis externo.
**Ubicación**: Botón “Exportar CSV” en la esquina superior derecha del encabezado, visible para Dirección, Secretaría y Administración.
...
- **Validaciones**: El CSV respetará los filtros vigentes.
...
- **Validaciones**: El sistema genera un archivo con encabezados fijos y datos sanitizados (comillas escapadas, reemplazo de saltos de línea) para evitar problemas de formato.
...
#### Casos Especiales:
- El archivo contiene máximo 1.000 caracteres por columna en descripción y acciones para mantener la legibilidad.
...
**Ubicación**: Botón **Nueva Acta** en el encabezado; abre un diálogo modal con el formulario correspondiente.
...
- **Validaciones**: El sistema carga datos auxiliares (alumnos disponibles, personal, secciones) antes de mostrar el formulario. Mientras carga aparece “Cargando información…”.
...
- **Validaciones**: Si no se selecciona un alumno aparece mensaje “Seleccioná un alumno de la lista.” y no se permite enviar.
...
- **Validaciones**: Se verifica que la fecha esté entre hoy y dos días atrás; los campos de hora, lugar, descripción y acciones no pueden quedar vacíos o con espacios únicamente.
...
- **Validaciones**: En modo docente se intenta asignar automáticamente al propio docente; si no es posible aparece mensaje “No se pudo determinar el docente informante.”. Dirección puede elegir cualquier docente registrado y un directivo firmante opcional.
...
- **Validaciones**: Si falta información se muestran toasts específicos (por ejemplo, “Completá hora, lugar, descripción y acciones.”). Si todo es correcto el diálogo se cierra automáticamente tras crear el registro.
...
- ❌ **Error**: Toasts como “Seleccioná un alumno de la lista.”, “Fecha inválida: solo hoy o 2 días previos.”, “Completá hora, lugar, descripción y acciones.”, “Seleccioná un docente informante.” o “No se pudo crear el acta.” según la validación fallida.
- ⚠️ **Advertencia**: Mensajes informativos cuando no hay docentes disponibles para seleccionar.
...
- Docentes sólo ven alumnos de sus secciones vigentes; en caso de no existir asignaciones se incluye un fallback con todos los alumnos para no bloquear la carga.

### 4.5 Editar un acta existente
**Descripción**: Actualiza datos de un acta aún no firmada (Dirección/Secretaría).
**Ubicación**: Botón **Editar** en cada fila del listado y opción **Editar** en el diálogo de visualización.
...
- **Validaciones**: Las actas firmadas no pueden editarse; aparece advertencia “El acta ya está firmada y no puede editarse.”.
...
- **Validaciones**: Debe existir informante asignado. Campos obligatorios validan presencia de datos. Para seleccionar firmante, el acta no puede estar en estado “Borrador”.
...
- **Datos requeridos**: Todos los campos obligatorios completos.
- **Validaciones**: Muestra toasts específicos si falta información (“Seleccioná un alumno válido.”, “El acta no tiene un docente informante asignado.”, etc.).
- **FOTO-ADJUNTA**: Botón “Guardar cambios” resaltado.
- **Resultado esperado**: El diálogo se cierra y el listado se actualiza con los cambios guardados.

#### Mensajes del Sistema:
- ✅ **Éxito**: Cambios guardados sin notificación adicional (se refresca la lista principal al cerrarse el diálogo).
- ❌ **Error**: Toasts mencionados en las validaciones; errores de red muestran “No se pudo actualizar el acta”.
- ⚠️ **Advertencia**: Mensaje indicando que debe cerrar el acta antes de elegir dirección firmante cuando el estado es Borrador.

#### Casos Especiales:
- El campo “Dirección firmante” se bloquea si el acta sigue en borrador; primero debe cerrarse para habilitarlo.
...
**Ubicación**: Botón **Ver** en cada fila del listado. Abre el diálogo de detalle con pestañas informativas.
...
- **Resultado esperado**: Se despliega el detalle del acta con información del alumno, suceso y seguimiento.
...
- **Datos requeridos**: Ninguno.
- **Validaciones**: Si ocurre un error al generar el PDF aparece toast “No se pudo generar el PDF del acta.”.
...
- ❌ **Error**: Toast con mensaje descriptivo ante fallos de generación.
- ⚠️ **Advertencia**: Mensajes informativos dentro del diálogo (por ejemplo, recordatorios sobre cerrar el acta para asignar firmante).
...
**Ubicación**: Botones dentro del diálogo de visualización y selectores correspondientes.
...
- **Validaciones**: Si falta información aparece “El acta no tiene información completa. Editá sus datos antes de cerrar el acta.” o “El acta no tiene asignado un alumno o docente responsable.”.
- **FOTO-ADJUNTA**: Botón “Cerrar acta” habilitado.
- **Resultado esperado**: Cambio de estado a “Cerrada” y toast “Acta cerrada”.
...
- **Validaciones**: No se permite asignar firmante si el acta sigue en borrador; muestra “Cerrá el acta antes de asignar una dirección firmante.”.
- **FOTO-ADJUNTA**: Selector abierto con la lista de direcciones disponibles.
- **Resultado esperado**: Toast “Dirección firmante actualizada” y actualización inmediata de la información del acta.
...
- **Validaciones**: Si faltan requisitos se muestran toasts como “Primero cerrá el acta antes de marcarla como firmada.”, “Asigná una dirección firmante antes de marcar el acta como firmada.” o “El acta no tiene asignado un alumno o docente responsable.”.
- **FOTO-ADJUNTA**: Botón “Firmar acta” habilitado.
- **Resultado esperado**: Toast “Acta marcada como firmada” y actualización del estado a “Firmada”.

#### Mensajes del Sistema:
- ✅ **Éxito**: “Acta cerrada”, “Dirección firmante actualizada”, “Acta marcada como firmada”.
- ❌ **Error**: “No se pudo actualizar el acta” ante fallas de red o servidor.
- ⚠️ **Advertencia**: Toasts que indican pasos previos faltantes (cerrar acta, asignar firmante, completar datos).

#### Casos Especiales:
- El botón “Firmar acta” queda deshabilitado hasta que exista un firmante asignado y el estado sea “Cerrada”.

### 4.8 Eliminar un acta
**Descripción**: Quita un acta del sistema de forma permanente (Dirección/Secretaría).
**Ubicación**: Botón **Eliminar** en la fila del listado y en la barra de acciones del diálogo de visualización.
...
- **Validaciones**: Ventana de confirmación con la pregunta “¿Eliminar el acta seleccionada?”; si cancela, no se realiza ninguna acción.
...
- **Datos requeridos**: No aplica.
- **Validaciones**: Mientras se procesa, el botón muestra “Eliminando…”. Al finalizar, se muestra toast “Acta eliminada” y el registro desaparece del listado.
- **FOTO-ADJUNTA**: Listado actualizado sin el acta eliminada.
- **Resultado esperado**: El registro ya no figura en la lista.

#### Mensajes del Sistema:
- ✅ **Éxito**: “Acta eliminada”.
- ❌ **Error**: “No se pudo eliminar el acta” si ocurre un problema durante la operación.
- ⚠️ **Advertencia**: Ninguna adicional.

#### Casos Especiales:
- Si el acta estaba abierta en el diálogo de visualización, se cierra automáticamente tras la eliminación.
...
**Ubicación**: Se carga automáticamente cuando accede un usuario con alcance “family” o “student”.
...
- **Datos requeridos**: Ninguno.
- **Validaciones**: Si hay alumnos asociados, se listan automáticamente; en caso contrario se muestra un mensaje informativo.
...
- **Datos requeridos**: No aplica.
- **Validaciones**: Estados mostrados mediante insignias de color (Firmada, Cerrada, Borrador). Si no hay actas se muestra el texto “No se registraron actas para este alumno.”.
...
- ❌ **Error**: Texto en rojo “No se pudo obtener el listado de actas.” si la consulta falla.
- ⚠️ **Advertencia**: Mensaje “No hay alumnos asociados a esta cuenta.” cuando el perfil no tiene alumnos vinculados.

#### Casos Especiales:
- Mientras se cargan los datos se muestra el indicador “Cargando actas…”. Si ocurre un error se mantiene el mensaje hasta que se recargue la vista.

## 5. Preguntas Frecuentes
1. **¿Por qué no puedo editar un acta?** — Solo Dirección y Secretaría pueden editar. Además, las actas firmadas quedan bloqueadas para edición.
2. **¿Qué debo hacer si no veo alumnos en el formulario?** — Verifique que su rol tenga secciones asignadas. El sistema intentará cargar alumnos del período activo; si no hay asignaciones, contacte a la administración para actualizar los datos.
3. **¿Se puede registrar un acta con fecha anterior a dos días?** — No. Por seguridad solo se aceptan incidentes ocurridos hoy o en los dos días previos.
4. **¿Cómo identifico el estado del acta?** — Cada registro muestra una insignia: Borrador (rojo), Cerrada (gris) o Firmada (verde).

## 6. Solución de Problemas
- **Mensaje “No se pudo crear el acta.”**: Revise que todos los campos obligatorios estén completos y que el docente informante esté seleccionado. Si persiste, puede deberse a un error de conexión con el servicio de Vida Escolar.
- **Mensaje “No se pudo actualizar el acta.”** al cerrar/firmar: Compruebe que el acta tenga alumno, docente informante, hora, lugar, acciones y dirección firmante (para firmar).
- **No aparece el botón Nueva Acta**: Su rol no tiene permiso de creación (por ejemplo, perfiles administrativos sin rol autorizado o familias). Verifique con Dirección para que actualicen sus permisos.
- **Vista familiar vacía**: Confirme que el alumno esté asociado al usuario. Si no hay vínculo, contacte a la institución para actualizar los datos familiares.
# Alumnos – Aspirantes
...
## 2. Roles y Permisos
- **Usuarios con credenciales válidas**: pueden iniciar sesión completando el formulario y acceder según el rol asignado. Todos deben contar con un correo institucional válido para superar la verificación inicial. 
- **Roles habilitados en la plataforma**: Dirección, Administración, Secretaría, Coordinación, Docencia, Suplente, Familia, Alumno y Usuario. Estos roles determinan los menús y permisos una vez dentro del sistema. 
- **Selección de rol**: cuando la cuenta posee más de un rol, la persona debe escoger cuál utilizar antes de ingresar al panel. El sistema recuerda el rol elegido para futuras sesiones. 
- **Protección de áreas privadas**: las rutas `/dashboard` y `/select-rol` requieren tener la sesión abierta. En caso contrario, se redirige automáticamente a la pantalla de inicio de sesión. 
- **Falta de roles**: si la cuenta no tiene ningún rol disponible, se muestra una tarjeta informativa y sólo se ofrece cerrar la sesión. 
...
- **Validaciones**: Campo obligatorio, formato de correo electrónico válido. El sistema impide continuar si el campo queda vacío o con formato incorrecto. 
...
- **Datos requeridos**: Ninguno adicional.
- **Validaciones**: Mientras la verificación está en curso, el botón se deshabilita y muestra el mensaje "Verificando..." para evitar envíos duplicados. 
...
- ❌ **Error**: Notificación emergente "No pudimos verificar el correo electrónico" u otro mensaje enviado por el servidor (por ejemplo, si el correo no está registrado). 
- ⚠️ **Advertencia**: El sistema impide nuevos envíos mientras la verificación anterior está pendiente.

#### Casos Especiales:
- Si la sesión ya está iniciada con un rol seleccionado, la pantalla redirige automáticamente al panel sin mostrar el formulario. 
...
- **Validaciones**: Campo obligatorio. El texto de ayuda recuerda que debe tener mínimo 8 caracteres, 2 números y 1 símbolo especial. 
...
- **Datos requeridos**: Ninguno adicional.
- **Validaciones**: Durante el envío, el botón se deshabilita y muestra "Ingresando..." para evitar clics repetidos. 
...
- ❌ **Error**: Notificación emergente "Error al iniciar sesión" o la respuesta de error enviada por el servidor (por ejemplo, contraseña incorrecta). 
- ⚠️ **Advertencia**: Si la cuenta no tiene roles configurados, se muestra una tarjeta especial indicando que debe contactar a la administración. 

#### Casos Especiales:
- Si la cuenta sólo posee un rol, el sistema lo selecciona automáticamente y abre el panel principal. 
- Si existen varios roles, se abre la pantalla "Elegí con qué rol entrar" para que el usuario elija antes de continuar. 
...
- **Resultado esperado**: El texto del campo cambia entre modo oculto y visible. 
...
#### Casos Especiales:
- Si cambia nuevamente el correo (ver funcionalidad 4.4), la contraseña se limpia y vuelve a ocultarse automáticamente. 
...
- **Resultado esperado**: El campo de contraseña desaparece, la contraseña guardada se borra y el campo de correo vuelve a estar editable. 
...
#### Casos Especiales:
- Mientras se envía la contraseña, el botón permanece deshabilitado para evitar inconsistencias. 
...
- **Resultado esperado**: Se abre la página `/solicitud` para completar la postulación. 
...
#### Casos Especiales:
- Si el correo ya fue validado, el botón desaparece para evitar confusiones. 
...
- **Datos requeridos**: Ninguno.
- **Validaciones**: Sólo se muestra si la cuenta posee más de un rol y la sesión está activa. 
...
- **Resultado esperado**: Se ingresa a `/dashboard` con los permisos del rol elegido. 

#### Mensajes del Sistema:
- ✅ **Éxito**: No se muestra mensaje; la redirección es inmediata.
- ❌ **Error**: No aplica.
- ⚠️ **Advertencia**: Si el usuario intenta acceder sin haber iniciado sesión, es redirigido automáticamente a la pantalla de login. 

#### Casos Especiales:
- Si en un futuro sólo queda un rol disponible, la pantalla no se muestra y se ingresa directo al panel. 
...
- **Resultado esperado**: Se finaliza la sesión, se borran las credenciales guardadas y se regresa a la pantalla de login. 

#### Mensajes del Sistema:
- ✅ **Éxito**: No se muestra mensaje; la pantalla vuelve al formulario de inicio de sesión.
- ❌ **Error**: No aplica.
- ⚠️ **Advertencia**: Si la red se interrumpe, el sistema igualmente limpia la sesión local y recarga la página de inicio. 
...
#### Mensajes del Sistema:
- ⚠️ **Advertencia**: Texto en color rojo "No tenés permisos para acceder a esta sección." 
...
## 5. Preguntas Frecuentes
- **¿Por qué el sistema me pide validar el correo antes de la contraseña?** Para confirmar que la cuenta existe y guiar a los usuarios sin acceso hacia el formulario de solicitud. 
- **¿Qué sucede si tengo más de un rol?** Después de iniciar sesión se le mostrará la pantalla de selección de rol para elegir con cuál trabajar. El sistema recordará su última elección. 
- **¿Por qué vuelvo al inicio cuando intento entrar al panel directo?** Las áreas `/dashboard` y `/select-rol` están protegidas. Si la sesión venció o no está autenticado, el sistema lo envía nuevamente al login. 

## 6. Solución de Problemas
- **El botón queda en "Verificando..." y no avanza**: Compruebe su conexión a internet. Si persiste, recargue la página e intente nuevamente. Mientras el botón muestre ese texto, espere a que finalice la solicitud. 
- **Recibo el mensaje "No pudimos verificar el correo electrónico"**: Confirme que está usando su correo institucional. Si el problema continúa, comuníquese con el área administrativa para registrar su cuenta. 
- **El sistema indica "Sin roles asignados"**: Contacte a la administración para que le asignen un rol. Por el momento sólo puede cerrar la sesión desde esa tarjeta. 
- **Soy redirigido al inicio al intentar abrir `/select-rol`**: Esto ocurre si la sesión expiró o ingresó el enlace sin iniciar sesión. Vuelva a autenticarse y repita el proceso. 
- **Veo el mensaje de "No tenés permisos"**: Asegúrese de haber elegido el rol correcto o solicite permisos. Si el problema continúa, contacte al soporte técnico indicando la sección a la que quiso acceder. 
# Calificaciones e Informes

## 1. Introducción
La sección **Calificaciones e Informes** permite consultar los resultados trimestrales de los estudiantes y, en el caso del personal docente o administrativo, registrar o actualizar las calificaciones finales y los informes descriptivos que se entregan a las familias. La vista se adapta automáticamente al rol con el que se ingresa al sistema para mostrar únicamente las acciones habilitadas para cada perfil.

## 2. Roles y Permisos
- **Familias y estudiantes**: pueden acceder en modo lectura para revisar calificaciones finales, observaciones y los informes descriptivos cargados por trimestre.
- **Docentes asignados y personal administrativo**: gestionan las calificaciones de nivel primario y los informes de nivel inicial. El acceso docente se limita a las secciones asignadas; otras secciones permanecen ocultas.
- **Administradores**: el perfil de Administración no tiene acceso a esta sección y recibe un mensaje 403 si intenta ingresar.
...
- **Resultado esperado**: El sistema redirige a `/dashboard/calificaciones`, cargando la vista correspondiente al rol del usuario.
...
- **Resultado esperado**: Se actualiza el detalle académico del alumno elegido.
...
- **Resultado esperado**: Confirma la sección académica a la que pertenece el alumno.
...
- **Resultado esperado**: Se visualizan las calificaciones finales, la cantidad de registros y los comentarios por trimestre.
...
- **Resultado esperado**: Se leen los informes descriptivos o se identifica si están pendientes.

#### Mensajes del Sistema:
- ✅ **Éxito**: Visualización normal de las tarjetas cuando existen datos cargados.
- ❌ **Error**: "No se pudo cargar la información académica." cuando la carga en segundo plano falla.
- ⚠️ **Advertencia**:
  - "Cargando calificaciones…" mientras se obtienen los datos iniciales.
  - "No hay alumnos asociados a esta cuenta." si la familia no tiene estudiantes vinculados.
  - "Las calificaciones estarán disponibles una vez que la dirección cierre el trimestre." cuando aún no se publicaron los resultados.

#### Casos Especiales:
- Si la cuenta tiene varios alumnos, cada pestaña mantiene los datos cargados incluso al cambiar entre perfiles, salvo que se refresque la página.
- Para secciones sin materias registradas, se muestra un mensaje informativo en lugar de tarjetas.
...
- **Resultado esperado**: Se actualiza el listado de secciones mostradas.
...
- **Resultado esperado**: El sistema abre la página de gestión de la sección seleccionada.

#### Mensajes del Sistema:
- ✅ **Éxito**: Navegación a la página de sección correspondiente.
- ❌ **Error**: Si se produce un error al cargar las secciones se muestra un mensaje en rojo con el detalle devuelto por el sistema.
- ⚠️ **Advertencia**:
  - "Cargando secciones…" durante la carga inicial.
  - "No hay secciones de Primario/Inicial disponibles." cuando la lista está vacía.

#### Casos Especiales:
- Los docentes sin asignaciones reciben un mensaje 403 indicando que la sección no pertenece a sus asignaciones al intentar abrir un acceso directo externo.
...
- **Resultado esperado**: Se establece el trimestre activo para cargar notas.
...
- **Resultado esperado**: Se carga la grilla de alumnos para la materia seleccionada.
...
- **Resultado esperado**: Los valores se guardan temporalmente en la vista a la espera de confirmación.
...
#### Mensajes del Sistema:
- ✅ **Éxito**: "Calificaciones guardadas." tras una actualización correcta.
- ❌ **Error**: "No se pudo guardar." o el mensaje devuelto por el servidor cuando falla la operación.
- ⚠️ **Advertencia**:
  - "Cargando calificaciones…" mientras se cargan las filas o la tabla.
  - Alertas en color ámbar cuando el trimestre está cerrado o inactivo, indicando que la vista es solo de lectura.
  - Etiquetas "Calculando…" mientras se obtienen promedios de asistencia o exámenes.

#### Casos Especiales:
- Si no hay trimestres o materias configuradas, se muestra un mensaje orientativo y no se habilita la edición.
- Los indicadores de asistencia y promedio pueden quedar en blanco si no hay datos cargados; en ese caso se muestra "Sin registros".
- Cuando no existen alumnos inscriptos para la materia en el trimestre seleccionado, se muestra un mensaje y no aparece el botón de guardado.
...
- **Resultado esperado**: Se visualiza el contenido existente o las opciones para cargar un informe.
...
- **Resultado esperado**: El informe se guarda y el bloque pasa a mostrar la descripción registrada.
...
- **Resultado esperado**: Se actualiza la descripción y se cierra el diálogo. Si el backend no soporta la actualización, se muestra una advertencia indicando que se debe habilitar el endpoint correspondiente.

#### Mensajes del Sistema:
- ✅ **Éxito**: El bloque muestra inmediatamente el texto guardado después de crear o actualizar un informe.
- ❌ **Error**: "Tu backend aún no expone UPDATE para informes. Pedilo o habilítalo." cuando el servidor no permite editar informes existentes.
- ⚠️ **Advertencia**:
  - "Cargando informes…" durante la carga inicial.
  - Mensajes "Sin informe." o "Trimestre inactivo. Aún no habilitado." cuando el trimestre está cerrado o inactivo.

#### Casos Especiales:
- Si el trimestre está cerrado, los datos se muestran en modo lectura y no es posible crear ni editar informes.
- Cada vez que se guarda un informe, la lista se actualiza en memoria para mantener la información visible sin recargar la página.

## 5. Preguntas Frecuentes
- **¿Por qué veo "Pendiente" en lugar de una nota?**: Significa que la dirección aún no cerró el trimestre, por lo que las calificaciones todavía no están publicadas.
- **¿Cómo sé si puedo editar una calificación o informe?**: Los botones de edición solo aparecen cuando el trimestre está activo; si se muestra un badge con estado cerrado o inactivo, la vista es de solo lectura.
- **¿Qué información adicional se muestra a los docentes?**: Para cada alumno se incluye el promedio de asistencia y de exámenes cuando hay datos disponibles, ayudando a contextualizar la calificación final.

## 6. Solución de Problemas
- **Mensaje 403 al ingresar**: Verifique que su rol tenga acceso. Los administradores no pueden utilizar esta sección y los docentes solo ven las secciones asignadas.
- **No aparecen secciones en el listado**: Puede deberse a que aún no se configuraron secciones para su rol o nivel. Si el problema persiste, comuníquese con la administración para revisar sus permisos.
- **Error al guardar calificaciones**: Revise la conexión y vuelva a intentar. Si el mensaje persiste, informe el detalle mostrado en pantalla para que soporte pueda revisarlo en el backend.
- **No se pueden editar informes existentes**: Si aparece el aviso sobre la falta de endpoint de actualización, solicite al equipo técnico habilitar el método PUT en el backend antes de reintentar.
# Comunicados
...
## 2. Roles y Permisos
- **Todos los usuarios autenticados** (personal, docentes, estudiantes y familias) pueden acceder a la sección y visualizar los comunicados visibles según su relación con la institución.
- **Director, Administrador, Secretaria y Coordinador** pueden crear y eliminar cualquier comunicado institucional, por nivel o por sección.
- **Docentes (titulares o suplentes)** pueden crear comunicados únicamente dirigidos a sus secciones asignadas y eliminar aquellos que hayan sido publicados para ellas.
- **Estudiantes y familias** solo pueden visualizar los comunicados que les correspondan; no tienen acciones de creación ni eliminación.
...
- **Resultado esperado**: Se abre la vista de comunicados con el listado, la barra de búsqueda y los filtros disponibles.
...
- **Resultado esperado**: Visualiza cada tarjeta con título, alcance, fecha y un resumen del cuerpo del comunicado.
...
- **Resultado esperado**: El listado se actualiza mostrando solo los comunicados cuyo título, cuerpo, alcance, nivel o sección coinciden con el término buscado.
...
- **Resultado esperado**: El listado muestra únicamente los comunicados cuyo alcance coincide con la opción elegida.
...
- **Resultado esperado**: Se abre un diálogo con el comunicado completo, incluyendo título, alcance, fecha y texto íntegro. Si su rol lo permite, encontrará el botón para eliminarlo desde esta vista.

#### Mensajes del Sistema:
- ✅ **Éxito**: Al cargar correctamente se muestran las tarjetas o el mensaje “No hay comunicados para mostrar”.
...
- ⚠️ **Advertencia**: Mensaje “Cargando comunicados…” mientras se recupera la información.

#### Casos Especiales:
- Si no existen comunicados visibles para su rol o relación con la institución, verá el mensaje “No hay comunicados para mostrar”.
- Los comunicados muestran el alcance como insignias: “Institucional”, “Nivel {nombre}” o el nombre de la sección asignada.
...
- **Resultado esperado**: Se abre un diálogo con el formulario de publicación.
...
- **Resultado esperado**: Los campos quedan guardados en pantalla y habilitan la selección de alcance.
...
- **Resultado esperado**: El formulario queda completo y el botón “Publicar” se habilita.
...
- **Resultado esperado**: El comunicado se publica, el formulario se cierra y el listado se actualiza automáticamente. Si ocurre un error, se muestra una notificación indicando que no se pudo crear el comunicado.

#### Mensajes del Sistema:
- ✅ **Éxito**: El diálogo se cierra y el nuevo comunicado aparece en el listado actualizado (no se muestra un mensaje, pero el resultado visible confirma la publicación).
- ❌ **Error**: “No se pudo crear el comunicado” cuando ocurre un problema en el envío; el sistema muestra detalles adicionales si están disponibles.
- ⚠️ **Advertencia**: El botón “Publicar” se deshabilita y muestra un ícono giratorio mientras se envía la información.

#### Casos Especiales:
- Al abrir el formulario, los campos se reinician para evitar que se publiquen datos anteriores por error.
- Si no posee permisos de creación, el botón “Nuevo Comunicado” no se muestra.
...
- **Resultado esperado**: Confirma que tiene permisos para eliminar ese comunicado.
...
- **Resultado esperado**: El comunicado desaparece del listado. Si ocurre un problema, se muestra una notificación de error.

#### Mensajes del Sistema:
- ✅ **Éxito**: El comunicado deja de aparecer en la lista tras confirmar la eliminación.
- ❌ **Error**: “No se pudo eliminar el comunicado” cuando la operación falla, mostrando detalles adicionales si están disponibles.
- ⚠️ **Advertencia**: El cuadro de confirmación indica que la acción es un borrado lógico y no puede deshacerse visualmente.

#### Casos Especiales:
- Los docentes solo pueden eliminar comunicados dirigidos a sus propias secciones con alcance “Por sección”. No podrán borrar comunicados institucionales ni de otras secciones.

## 5. Preguntas Frecuentes
1. **¿Por qué no veo el botón “Nuevo Comunicado”?**  
   Su rol no tiene permisos de publicación (solo directivos, administrativos, coordinadores y docentes pueden crear comunicados).
2. **¿Por qué algunos comunicados dicen “Institucional” y otros muestran una sección?**  
   Indican el alcance definido por quien lo publicó: institucional para toda la comunidad, por nivel (Inicial/Primario) o por sección específica.
3. **¿Qué sucede si la lista está vacía?**  
   Significa que no hay comunicados visibles para usted en este momento; aparecerá el mensaje correspondiente.
...
## 6. Solución de Problemas
- **No se cargan los comunicados y solo veo “Cargando comunicados…”**: Verifique su conexión. El sistema intenta recuperar la información automáticamente; si persiste, recargue la página.
- **Aparece el mensaje “No se pudo crear el comunicado”**: Revise su conexión o vuelva a intentarlo más tarde. Si el problema continúa, contacte al área de sistemas con el detalle mostrado en la notificación.
- **Aparece el mensaje “No se pudo eliminar el comunicado”**: Ocurre cuando la eliminación falla. Confirme que mantiene permisos sobre el comunicado e intente nuevamente; si persiste, solicite soporte técnico.
# Dashboard - Configuración

## 1. Introducción
La ventana **Configuración** centraliza los ajustes personales y administrativos del panel. Desde aquí puede activar el modo oscuro, administrar trimestres y períodos escolares, y definir las credenciales del correo institucional según el rol con el que haya iniciado sesión.

## 2. Roles y Permisos
- **Todos los roles**: acceden a la pestaña **Apariencia** para alternar el modo oscuro.
- **Rol Dirección**: habilita las pestañas **Trimestres**, **Período escolar** y **Correo Electrónico**. Si no selecciona este rol, verá un mensaje que le indica que cambie a “Dirección” para continuar.
- Cuando no existe información adicional para su rol (por ejemplo, si no es Dirección) se muestra un aviso informativo.
...
- **Resultado esperado**: Se despliega el menú de usuario con las opciones de cambio de rol y configuración.
...
- **Resultado esperado**: Se abre el diálogo de Configuración con la pestaña **Apariencia** activa.
...
- **Resultado esperado**: Identifica si el modo oscuro está activado (interruptor a la derecha) o desactivado (a la izquierda).
...
- **Resultado esperado**: La interfaz cambia al modo seleccionado inmediatamente.

#### Mensajes del Sistema:
- No se muestran mensajes emergentes; el cambio es visual e inmediato.

#### Casos Especiales:
- El interruptor se deshabilita brevemente al abrir la ventana hasta que se determine el tema actual.
...
- **Resultado esperado**: La pestaña “Trimestres” se habilita y muestra la gestión correspondiente.
...
- **Resultado esperado**: Se muestran los trimestres del período seleccionado junto con su estado y botones de acción.
...
- **Resultado esperado**: Los campos muestran las fechas elegidas y el botón **Guardar cambios** se habilita.
...
- **Resultado esperado**: Se muestra una notificación indicando el resultado y, si es correcto, la lista se actualiza automáticamente.
...
- **Resultado esperado**: El estado cambia y aparece la insignia correspondiente (Activo, Inactivo o Cerrado).
...
- ⚠️ **Advertencia**:
  - Los botones muestran indicaciones al pasar el cursor (por ejemplo, “Este trimestre ya está activo”) cuando la acción no está disponible.
...
- Si la carga inicial falla, se mostrará “No se pudo cargar el calendario escolar”.
...
- **Resultado esperado**: Visualiza el período activo, su estado y botones de acción para cada registro.
...
- **Resultado esperado**: El estado del período se actualiza y se muestra un mensaje de confirmación.
...
- **Resultado esperado**: Se crea el nuevo período y el formulario propone automáticamente el año siguiente.
...
- **Resultado esperado**: Los campos muestran los valores guardados y, si existe una contraseña registrada, el campo aparece oculto con puntos.
...
- **Resultado esperado**: Todos los campos válidos quedan listos para guardar.
...
- **Resultado esperado**: El sistema confirma la actualización y, si cambió la contraseña, esta se limpia del formulario tras guardar.
...
- ⚠️ **Advertencia**:
  - Si la autenticación está deshabilitada, los campos de usuario y contraseña quedan inactivos y se muestra una nota aclaratoria.
...
- STARTTLS está pensado para conexiones seguras (por ejemplo, puerto 587) según la descripción incluida en la interfaz.

## 5. Preguntas Frecuentes
1. **¿Por qué no veo las pestañas de Trimestres, Período o Correo?** Debe elegir el rol **Dirección** en el menú de usuario para habilitarlas.
2. **¿Qué ocurre si dejo el campo Contraseña vacío?** Se mantiene la contraseña actual siempre que no haya marcado un nuevo valor.
3. **¿Puedo crear más de un período activo a la vez?** No, al activar un período el actual se cerrará automáticamente.

## 6. Solución de Problemas
- **Error al cargar datos del calendario**: Revise su conexión y vuelva a abrir la pestaña; el sistema mostrará “No se pudo cargar el calendario escolar” si falla la descarga.
- **Mensajes de fechas inválidas**: Ajuste las fechas asegurándose de no superponerlas y de respetar el orden cronológico requerido por el sistema.
- **Validación de puerto SMTP**: Use solo números entre 1 y 65.535 para evitar los mensajes “Ingresá un puerto válido” o “El puerto debe estar entre 1 y 65535”.
- **No se pudo guardar la configuración**: Utilice el botón **Recargar** para recuperar los datos previos y vuelva a intentar guardarlos.
# Gestión de Exámenes
...
- **Edición de datos personales y complementarios**: disponible para el mismo personal que accede a la sección; no hay restricciones adicionales en la interfaz.
- **Gestión de credenciales (crear/actualizar acceso)**: cualquier usuario autorizado puede abrir el cuadro de diálogo y actualizar email y contraseña. Sin embargo, solo los roles **Administrador** y **Director** pueden modificar la lista de roles del familiar dentro del sistema.

> **Nota:** Si su rol no permite editar los roles del familiar, la sección de selección de roles no aparecerá y el sistema conservará los roles actuales.
...
- **Resultado esperado**: Se abre la vista “Perfil del Familiar” con el encabezado que muestra el ID del familiar y el botón «Volver» en la parte superior.
...
**Ubicación**: Secciones “Datos personales” e “Información familiar” en la parte superior del perfil.
...
- **Validaciones**: Si algún dato no existe, el sistema mostrará un guion largo (—) en su lugar.
...
- **Validaciones**: Si no se registró información, se mostrará un guion largo (—).
...
#### Mensajes del Sistema:
- ⚠️ **Advertencia**: Cuando los datos no están disponibles, se muestran con el símbolo «—» para indicar ausencia de información.

#### Casos Especiales:
- Si el sistema aún está cargando la información, se mostrará el mensaje «Cargando información del familiar…».
- Si ocurre un problema al obtener los datos, verá un mensaje en rojo con la leyenda «No pudimos cargar la información del familiar» o el detalle del error devuelto por el sistema.
...
**Ubicación**: Tarjeta “Alumnos vinculados” en la parte central/inferior del perfil.
...
- **Datos requeridos**: No requiere ingreso de datos.
- **Validaciones**: Si no hay alumnos, el sistema mostrará el mensaje «No hay alumnos asociados a este familiar.»
...
- **Resultado esperado**: Se abre la página del alumno seleccionado en una nueva vista del panel.

#### Mensajes del Sistema:
- ✅ **Indicadores visuales**: Si el vínculo posee un rol, aparece una insignia (badge) con el tipo de relación; si está marcada la convivencia, se muestra la insignia “Convive”.
- ⚠️ **Advertencia**: Mensaje «No hay alumnos asociados a este familiar.» cuando no existen registros vinculados.

#### Casos Especiales:
- Si el sistema no encuentra vínculos, solo verá el mensaje informativo y no habrá botones disponibles para abrir perfiles de alumnos.
...
**Ubicación**: Botón «Editar datos» en la esquina superior derecha del perfil.
...
- **Resultado esperado**: Se abre el cuadro modal “Editar datos del familiar” con los campos precargados.
...
- **Validaciones**:
  ...
  - Si omite Nombre o Apellido, el sistema mostrará el error correspondiente al intentar guardar.
...
- **Resultado esperado**: Si la información es correcta, el cuadro se cerrará y los datos se actualizarán en la pantalla.

#### Mensajes del Sistema:
- ✅ **Éxito**: «Datos del familiar actualizados» cuando la actualización se realiza correctamente.
- ❌ **Error**:
  - «No encontramos los datos del familiar para editar» si falta información base.
  - «Completá nombre y apellido» cuando deja vacíos ambos campos.
  - «Ingresá un DNI válido (7 a 10 dígitos).» si el documento no cumple el formato.
  - Mensaje dinámico o «No pudimos actualizar los datos del familiar» si se produce un error del servidor.

#### Casos Especiales:
- Si el familiar no tiene persona asociada completa, la interfaz rellena los campos con un identificador y permite completarlos manualmente al editar.
- Tras guardar, la página se recarga automáticamente para reflejar los cambios más recientes.
...
**Ubicación**: Tarjeta “Acceso al sistema” al final de la página.
...
- **Resultado esperado**: Se abre el cuadro modal con los campos de email, contraseña y confirmación.
...
- **Validaciones**:
  ...
  - Los roles “Familiar” y “Estudiante” son excluyentes entre sí; al elegir uno, el otro se desmarca automáticamente.
...
- **Resultado esperado**: El modal se cierra, se actualiza la información y se muestran los datos de acceso en la tarjeta principal.

#### Mensajes del Sistema:
- ✅ **Éxito**: «Acceso del familiar actualizado» cuando se guardan las credenciales con éxito.
- ❌ **Error**:
  - «No encontramos la persona vinculada» si faltan datos esenciales.
  - «Ingresá un email válido» si deja el correo vacío.
  - «Definí una contraseña inicial» cuando intenta crear acceso sin contraseña.
  - «Las contraseñas no coinciden» si la confirmación difiere.
  - «Seleccioná al menos un rol para el acceso» cuando no hay roles elegidos.
  - Mensaje dinámico o «No pudimos actualizar el acceso del familiar» si ocurre un error del servidor.

#### Casos Especiales:
- Cuando el familiar ya posee credenciales activas, el campo contraseña es opcional y el botón se muestra como «Actualizar acceso».
- Si no tiene credenciales, el sistema indica «El familiar todavía no tiene credenciales asignadas.» y el botón dice «Crear acceso».

## 5. Preguntas Frecuentes
1. **¿Qué significa el guion largo (—) en un campo?**
   - Indica que aún no se registró información para ese dato.
2. **¿Por qué no veo la sección para elegir roles?**
   - Solo los usuarios con rol Administrador o Director pueden modificar los roles del familiar.
3. **¿Qué ocurre si no hay alumnos vinculados?**
   - La tarjeta lo indicará con el mensaje “No hay alumnos asociados a este familiar.” y no se mostrarán botones adicionales.
...
- **No puedo guardar cambios porque falta el DNI**: Verifique que el DNI tenga entre 7 y 10 dígitos y solo incluya números.
- **El sistema pide definir una contraseña inicial**: Esto ocurre cuando el familiar no tenía credenciales activas. Ingrese una contraseña segura y vuelva a intentarlo.
- **Las contraseñas no coinciden al guardar el acceso**: Asegúrese de escribir la misma contraseña en ambos campos antes de presionar «Guardar acceso».
- **Necesito asignar ambos roles Familiar y Estudiante**: No es posible; el sistema desmarca automáticamente el rol incompatible para evitar conflictos.
...
- **Resultado esperado**: Visualiza monto, vencimiento, estado y código de pago.
...
- **Resultado esperado**: Se abre un diálogo con alumno, sección, vencimiento, importes, recargo y observaciones.
...
- **Resultado esperado**: Se copia el código al portapapeles y aparece la notificación **"Código de pago copiado"**.

#### Mensajes del Sistema:
- ✅ **Éxito**:
  - "Código de pago copiado" al copiar un código.
- ❌ **Error**:
  - "No se pudo obtener la información" al fallar la carga de cuotas familiares.
  - "No se pudo obtener los pagos registrados" cuando falla la carga administrativa.
  - "No se pudo copiar el texto" si el portapapeles no está disponible.
- ⚠️ **Advertencia**:
  - Tarjetas muestran estados "Vencida", "Pagada", "Pago parcial" o "Vigente" según la lógica de vencimiento.

#### Casos Especiales:
- Si no hay alumnos asociados a la familia, se muestra el mensaje "Aún no hay alumnos asociados a tu cuenta".
- Si no hay cuotas cargadas, se informa "No hay cuotas registradas aún" o "Aún no se registraron cuotas" según la vista.
...
- **Resultado esperado**: Se abre el diálogo "Nueva cuota" con el formulario dividido en secciones.
...
- **Resultado esperado**: Las secciones elegidas aparecen marcadas.
...
- **Resultado esperado**: Los campos muestran la configuración del período.
...
- **Resultado esperado**: Los campos quedan completos listos para enviar.
...
- **Resultado esperado**: Se generan las cuotas y aparece la notificación "Se generaron X cuotas" o "No se generaron nuevas cuotas (posibles duplicados)". El formulario se cierra y el listado se actualiza.

#### Mensajes del Sistema:
- ✅ **Éxito**:
  - "Se generaron X cuotas" cuando la operación crea nuevas cuotas.
  - "No se generaron nuevas cuotas (posibles duplicados)" si no se crean registros pero la solicitud fue válida.
- ❌ **Error**:
  ...
  - "No se pudo crear la cuota" con detalle del servidor si falla la solicitud.
- ⚠️ **Advertencia**:
  - Información contextual: "Las cuotas de matrícula no requieren selección de mes…" en el formulario.

#### Casos Especiales:
- Si no existen secciones cargadas, se muestra "No hay secciones disponibles para generar cuotas" dentro del panel de selección.
- El porcentaje de recargo se aplica automáticamente para pagos fuera de término; el detalle se visualiza en el modal de cuota.
...
- **Resultado esperado**: Se muestra el formulario con el campo *Tipo de registro* seleccionado en "Pago de cuota" por defecto.
...
- **Resultado esperado**: El formulario presenta los campos de cuotas disponibles para el tipo elegido.
...
- **Resultado esperado**: El campo queda poblado con la cuota seleccionada.
...
- **Resultado esperado**: Todos los campos quedan completos y listos para guardar.
...
- **Resultado esperado**: Se registra el pago, el diálogo se cierra y aparece la notificación "Pago registrado correctamente". Se actualizan los listados de cuotas y pagos.

#### Mensajes del Sistema:
- ✅ **Éxito**:
  - "Pago registrado correctamente" al guardar un pago válido.
- ❌ **Error**:
  - "Seleccioná una cuota" si no se elige registro asociado.
  - "Ingresá un monto válido" cuando el importe es incorrecto.
  - "No se pudo registrar el pago" si la solicitud al servidor falla, mostrando el detalle devuelto.
- ⚠️ **Advertencia**:
  - Sin cuotas disponibles aparece "No hay cuotas disponibles" dentro del selector.

#### Casos Especiales:
- Si se registra un pago sin cuota (por procesos especiales) el listado lo mostrará como "Pago sin cuota" sin detalles de alumno.
- La fecha puede quedar "Sin registrar" si no se carga en el formulario.
...
- **Resultado esperado**: Aparece la lista de pagos ordenados por fecha.
...
- **Resultado esperado**: El estado se actualiza y aparece la notificación "Estado de pago actualizado".

#### Mensajes del Sistema:
- ✅ **Éxito**: "Estado de pago actualizado" tras cambiar el estado.
- ❌ **Error**: "No se pudo actualizar el pago" si la operación falla.
- ⚠️ **Advertencia**: Los estados visibles en las insignias (En revisión, Acreditado, Rechazado) indican la situación actual.

#### Casos Especiales:
- La fecha de acreditación se guarda automáticamente al marcar **Acreditar** (no requiere ingreso manual).
...
- **Resultado esperado**: El formulario se abre con el tipo "Recibo de sueldo" seleccionado automáticamente.
...
- **Resultado esperado**: La información queda lista para guardar.
...
- **Resultado esperado**: Se registra el recibo y se muestra la notificación "Recibo de sueldo registrado"; el listado se actualiza.

#### Mensajes del Sistema:
- ✅ **Éxito**: "Recibo de sueldo registrado" al completar la operación.
- ❌ **Error**:
  ...
  - "No se pudo registrar el pago" si la solicitud falla (mensaje general para todo el formulario).
- ⚠️ **Advertencia**: El selector indica "No hay empleados disponibles" cuando no hay registros para elegir.

#### Casos Especiales:
- Tras guardar, el tipo de formulario vuelve a "Pago de cuota"; si desea cargar otro recibo, seleccione nuevamente la opción correspondiente.
...
- **Resultado esperado**: Se muestra el listado filtrado (todos los recibos para ADMIN, solo propios para docentes).
...
- **Resultado esperado**: Se copia el identificador y aparece "Identificador de archivo copiado".
...
- **Resultado esperado**: Cambia el estado y se muestra el mensaje "Recibo confirmado" o "Confirmación eliminada".
...
- **Resultado esperado**: Se abre el diálogo de pagos con el tipo "Recibo de sueldo" precompletado con los datos del registro seleccionado.

#### Mensajes del Sistema:
- ✅ **Éxito**:
  - "Identificador de archivo copiado" al copiar un comprobante.
  - "Recibo confirmado" o "Confirmación eliminada" según la acción realizada.
- ❌ **Error**:
  - "El recibo no tiene información suficiente para actualizarse" si faltan datos clave.
  - "No se pudo actualizar el recibo" cuando la operación falla.
- ⚠️ **Advertencia**:
  - Si no hay recibos disponibles se informa "Todavía no tenés recibos disponibles" o "Aún no se registraron recibos" según el rol.

#### Casos Especiales:
- Los docentes solo ven sus propios recibos gracias al filtrado automático; no necesitan realizar búsquedas adicionales.
...
2. **¿Puedo registrar pagos parciales?**
   - Sí, ingrese el monto abonado en el formulario. El estado de la cuota reflejará "Pago parcial" si el importe no cubre el total.
...
4. **¿Qué medios de pago admite el sistema?**
   - El campo **Medio de pago** lista todas las opciones configuradas (efectivo, transferencia, etc.) según la enumeración del sistema.

## 6. Solución de Problemas
- **Los listados no cargan y veo un mensaje de error**: Refresque la página. Si persiste, contacte al administrador para revisar el servicio indicado en el mensaje (cuotas, pagos o recibos).
- **No puedo copiar el código de pago**: Verifique que el navegador permita acceso al portapapeles. El sistema mostrará "No se pudo copiar el texto" si la acción es bloqueada.
- **No aparecen cuotas para seleccionar al registrar un pago**: Confirme que existan cuotas vigentes del tipo elegido (cuota o matrícula). Si no hay registros verá el aviso dentro del desplegable.
- **No puedo confirmar mi recibo**: Asegúrese de que el recibo contenga todos los datos (empleado, período, montos). Si falta información, avise a administración para completarla antes de confirmar.
...
## 2. Roles y Permisos
- **Director/a y Administrador/a**: pueden ver la información, aplicar filtros y registrar nuevas licencias. 
- **Secretaría**: puede ver la información y registrar nuevas licencias. 
...
- **Resultado esperado**: Se muestra la página con las pestañas **Personal** y **Licencias**. 
...
- **Resultado esperado**: Se despliegan los filtros y el listado de licencias (o el mensaje correspondiente si no hay datos). 
...
- **Descripción**: Ajusta la búsqueda mediante un campo de texto y selectores para nivel, sección, asignatura, cargo y situación, además de botones para limpiar filtros o actualizar los datos. 
...
- **Datos requeridos**: Texto libre.
- **Validaciones**: No aplica; el sistema ignora espacios al inicio y final. 
...
- **Validaciones**: Cada selector muestra únicamente valores presentes en los datos cargados. 
...
- **Validaciones**: El botón “Limpiar filtros” solo se habilita cuando hay criterios activos. “Actualizar datos” se deshabilita durante la carga. 
...
- ❌ **Error**: Si ocurre un problema al cargar los datos se muestra el mensaje “Error al cargar la información” con el botón **Reintentar**. 
- ⚠️ **Advertencia**: Cuando no se encuentran resultados, se muestran los mensajes “No se encontraron licencias con los criterios seleccionados.” o “Aún no se registraron licencias.” según corresponda. 

#### Casos Especiales:
- Durante la carga se muestra el estado “Cargando información…”. 
- Si la búsqueda no coincide con ningún registro y existen filtros activos, el botón **Limpiar filtros** permite restablecer rápidamente la vista. 
...
- **Rol requerido**: Todos los roles con acceso a la sección
- **Descripción**: Cada tarjeta muestra datos del personal, tipo y justificación de la licencia, fechas, horas de ausencia, motivo, observaciones, secciones y materias afectadas, además de la situación laboral visible. 
...
- **Datos requeridos**: No aplica.
- **Validaciones**: Las tarjetas se muestran ordenadas por las fechas guardadas (más recientes primero). 
...
- **Datos requeridos**: No aplica.
- **Validaciones**: Si no se registró fecha de finalización se mostrará únicamente la fecha de inicio; las horas de ausencia solo aparecen cuando se cargaron valores numéricos. 
...
- **Datos requeridos**: No aplica.
- **Validaciones**: El motivo siempre está presente; las observaciones se muestran solo si fueron cargadas. Las secciones y materias se listan con etiquetas diferenciadas. 
...
- **Validaciones**: Si la situación visible difiere de la situación original del legajo, se muestra entre paréntesis el valor registrado. 
...
- ⚠️ **Advertencia**: Si no hay licencias registradas se muestra el mensaje “Aún no se registraron licencias.” 
- ❌ **Error**: Si la página no puede cargar los datos se presenta el mensaje general de error descrito en la funcionalidad 4.1. 

#### Casos Especiales:
- Las tarjetas incluyen insignias que identifican si la licencia está justificada (gris) o sin justificar (rojo). 
- Se muestran etiquetas con el nivel y la sección afectada, así como las materias donde aplica la licencia. 
...
- **Descripción**: Abre un diálogo con formulario para seleccionar al personal, definir el tipo de licencia, fechas, motivo, justificación, horas de ausencia y observaciones adicionales. 
...
- **Resultado esperado**: Se abre el diálogo “Nueva licencia”. 
...
- **Validaciones**: Campo obligatorio. Si se ingresa desde la ficha del personal, el sistema preselecciona automáticamente al integrante correspondiente. 
...
- **Validaciones**: La fecha de inicio no puede quedar vacía. El sistema evita que la fecha de fin sea anterior a la de inicio ajustando automáticamente los valores y mostrando un error si se intenta guardar un rango inválido. 
...
- **Validaciones**: El motivo es obligatorio; el selector está predefinido en “Sí”. 
...
- **Validaciones**: El campo de horas solo acepta números iguales o superiores a cero; ambos campos son opcionales. 
...
- **Resultado esperado**: El diálogo se cierra y la nueva licencia aparece en el listado actualizado. 

#### Mensajes del Sistema:
- ✅ **Éxito**: “Licencia registrada” con la descripción “La licencia se registró correctamente.” 
- ❌ **Error**: Mensajes de validación preventiva: “Seleccione personal”, “Tipo requerido”, “Fecha requerida”, “Motivo requerido”, “Fechas de licencia inválidas”. Mensaje de error general: “Error al registrar licencia” con detalle de la causa. 
- ⚠️ **Advertencia**: Si no hay personal disponible para asignar, el selector muestra la opción deshabilitada “No hay personal disponible”. 

#### Casos Especiales:
- Al abrir el formulario desde una ficha específica, el campo **Personal** ya aparece completado con ese integrante. 
- Al cerrar el diálogo sin guardar, el formulario se restablece automáticamente y elimina cualquier dato parcial. 
- Si la fecha de fin se elimina, el sistema registra la licencia como de duración abierta hasta nuevo aviso. 

## 5. Preguntas Frecuentes
1. **¿Por qué no veo el botón “Nueva licencia”?**
   - Verifique que su usuario tenga rol de Dirección, Administración o Secretaría. 
2. **¿Qué sucede si no cargo la fecha de finalización?**
   - La licencia quedará abierta; el sistema mostrará solo la fecha de inicio hasta que se actualice. 
3. **¿Cómo vuelvo a ver todo el listado tras filtrar?**
   - Use el botón **Limpiar filtros** para restablecer los criterios. 

## 6. Solución de Problemas
- **No se cargan los datos de licencias**: Use el botón **Reintentar** del mensaje de error o **Actualizar datos** en el panel de filtros para volver a solicitar la información. 
- **No puedo guardar una licencia**: Revise que el formulario cumpla las validaciones (personal seleccionado, tipo, fecha de inicio y motivo obligatorios; fechas en orden). Corrija los campos resaltados y vuelva a intentar. 
- **El formulario muestra valores anteriores al reabrirlo**: Cierre completamente el diálogo; al volver a abrirlo los datos se reinician automáticamente. 
# Portal de Admisiones

## 1. Introducción
El Portal de Admisiones permite a las familias iniciar y completar la postulación de un aspirante en cinco pasos guiados, con validaciones en cada etapa, guardado automático de borradores y seguimiento posterior mediante la confirmación de entrevistas enviadas por la institución.

## 2. Roles y Permisos
- **Familias postulantes (sin autenticación previa):** pueden crear postulaciones, agregar familiares y enviar la solicitud siempre que autoricen el envío de comunicaciones por correo electrónico.
- **Familias con enlace de entrevista:** quienes reciben un correo con token y correo asociado pueden seleccionar o rechazar horarios de entrevista sin iniciar sesión.
...
- **Resultado esperado**: Se muestra un cuadro de diálogo solicitando el DNI del aspirante antes de iniciar el formulario.

## 4. Funcionalidades

### 4.1 Verificación de DNI del Aspirante
**Descripción**: Evita postulaciones duplicadas validando el DNI antes de permitir el acceso al formulario.
...
- **Validaciones**: Se rechazan DNIs vacíos, con longitud inválida o duplicados; los duplicados muestran el mensaje “El DNI ingresado es incorrecto o ya fue registrado previamente”.
- **FOTO-ADJUNTA**: Modal con el campo “DNI del aspirante”, el botón “Continuar” y el mensaje de ayuda.
- **Resultado esperado**: Si es válido, se habilita el formulario; si está repetido o es inválido, aparece un mensaje de error y no se avanza.

#### Mensajes del Sistema:
- ✅ **Éxito**: No aplica (el modal simplemente se cierra al validar).
- ❌ **Error**: “Ingresá un DNI válido de 7 a 10 dígitos.”; “El DNI ingresado es incorrecto o ya fue registrado previamente.”; “No se pudo verificar el DNI. Intentá nuevamente.”
- ⚠️ **Advertencia**: No hay advertencias específicas.

#### Casos Especiales:
- Si el usuario solicita cambiar el DNI luego de iniciada la carga, debe confirmar que reiniciará toda la postulación y se borrarán los datos guardados.

### 4.2 Paso 1 – Datos del Aspirante
**Descripción**: Recolecta información personal, académica y de contacto del aspirante.
...
- **Validaciones**: El DNI queda en solo lectura tras la verificación; la fecha debe ser al menos dos años anterior a la fecha actual.
...
- **Validaciones**: Deben elegirse opciones válidas de la lista.
...
- **Validaciones**: Si se omiten datos obligatorios se muestra un mensaje general al intentar avanzar.
...
- ❌ **Error**: Toast “Revisá los datos del aspirante.” con detalle de campos faltantes, formato de DNI o fecha inválida.
- ⚠️ **Advertencia**: Indicador textual “Verificando DNI…” mientras se consulta información previa.

#### Casos Especiales:
- Si el aspirante ya existe en la base, se autocompletan nombre, apellido, fecha, domicilio y nacionalidad luego de validar el DNI.

### 4.3 Paso 2 – Gestión de Familiares
**Descripción**: Permite agregar, editar o eliminar familiares responsables con sus datos de contacto y convivencia.
...
- **Validaciones**: Se impide agregar DNIs repetidos en la postulación; se muestran mensajes de error si es inválido o hubo un problema al consultar datos previos.
- **FOTO-ADJUNTA**: Diálogo “Agregar familiar” con campo DNI y alerta de error.
- **Resultado esperado**: Se crea una tarjeta “Familiar 1” con el DNI bloqueado para edición.
...
- **Validaciones**: Cada campo obligatorio se marca en rojo si queda vacío; el formulario exige fecha válida (≥2 años atrás), emails y teléfonos con formato numérico para DNI/teléfonos.
...
- **Validaciones**: Se exige al menos un familiar antes de avanzar; se muestra alerta si la lista queda vacía.
...
#### Mensajes del Sistema:
- ✅ **Éxito**: Toast “Datos básicos del familiar cargados…” o “Datos del familiar completados automáticamente.” al reutilizar registros existentes.
- ❌ **Error**: Toast “Completá los datos de al menos un familiar.” con detalle; alertas en el diálogo al ingresar un DNI inválido o repetido.
- ⚠️ **Advertencia**: Mensaje “No hay familiares agregados…” cuando la lista está vacía.

#### Casos Especiales:
- Si el familiar existe en el sistema con credenciales, se solicita usuario y contraseña para autocompletar datos; si no tiene credenciales, sólo se rellenan nombre y apellido y se informa que debe completar el resto manualmente.

### 4.4 Paso 3 – Condiciones del Hogar
**Descripción**: Registra la conectividad y los recursos tecnológicos del hogar del aspirante.
...
- **Validaciones**: Campo obligatorio; muestra borde rojo si se deja vacío.
...
- **Validaciones**: Ambos campos son obligatorios y requieren texto no vacío.
...
#### Mensajes del Sistema:
- ❌ **Error**: Toast “Completá la información del hogar.” si falta algún campo.

#### Casos Especiales:
- No aplica.

### 4.5 Paso 4 – Información de Salud
**Descripción**: Recoge antecedentes médicos y necesidades especiales del aspirante.
...
- **Validaciones**: No hay campos obligatorios, pero conviene completar los datos para el informe automático.
- **FOTO-ADJUNTA**: Textarea “Enfermedades o Alergias” con texto de ejemplo.
- **Resultado esperado**: La información se incorporará al resumen de observaciones de la solicitud.
...
- **Validaciones**: Sin restricciones adicionales.
...
#### Mensajes del Sistema:
- No se muestran mensajes específicos en este paso; los datos se validan al enviar la solicitud.

#### Casos Especiales:
- Si se ingresa información, se incluirá automáticamente en las observaciones enviadas al equipo de admisiones.

### 4.6 Paso 5 – Confirmación y Envío
**Descripción**: Resume los datos principales y solicita autorización para comunicaciones antes de enviar la postulación.
...
- **Validaciones**: Revise que toda la información sea correcta antes de continuar.
...
- **Validaciones**: La casilla debe estar marcada para habilitar el botón “Enviar Postulación”.
...
- **Datos requeridos**: Ninguno adicional.
- **Validaciones**: El sistema vuelve a comprobar fechas válidas y que exista al menos un familiar completo antes de enviar.
- **FOTO-ADJUNTA**: Barra con el botón “Enviar Postulación” resaltado.
- **Resultado esperado**: Se muestra una pantalla de agradecimiento con opciones para volver al inicio o iniciar una nueva postulación.

#### Mensajes del Sistema:
- ✅ **Éxito**: “Postulación enviada con éxito.” seguido del mensaje final en pantalla.
- ❌ **Error**: “Debes autorizar comunicaciones por email.”; “Fecha de nacimiento inválida” (aspirante o familiares); “Error al enviar: …” si ocurre un fallo en el guardado.

#### Casos Especiales:
- Tras enviar, se limpian los datos locales y el usuario puede iniciar una nueva postulación sin recargar la página.

### 4.7 Gestión de Borradores y Continuidad
**Descripción**: Guarda automáticamente el progreso y permite retomarlo o descartarlo.
...
- **Datos requeridos**: Información ingresada.
- **Validaciones**: El borrador se actualiza sólo si hay datos relevantes o si el usuario avanza de paso.
- **FOTO-ADJUNTA**: No aplica (se sugiere resaltar el formulario con indicador de borrador en la consola de navegador).
- **Resultado esperado**: Al recargar, aparece una alerta ofreciendo recuperar el borrador.
...
- **Validaciones**: Al recuperar, se restaura el paso y los datos; al descartar, se limpian y se reabre la verificación de DNI si corresponde.
- **FOTO-ADJUNTA**: Alerta mostrando ambos botones.
- **Resultado esperado**: El formulario se restaura o se reinicia según la opción.

#### Mensajes del Sistema:
- ✅ **Éxito**: “Borrador recuperado correctamente.” al restaurar.
- ⚠️ **Advertencia**: “Se descartó el borrador almacenado.” al optar por descartar.

#### Casos Especiales:
- Si se recupera un borrador con DNI registrado, el sistema mantiene el DNI bloqueado y salta el diálogo inicial; si no había DNI, el diálogo vuelve a mostrarse.

### 4.8 Cancelar o Reiniciar la Postulación
**Descripción**: Permite abandonar el proceso y borrar los datos locales en cualquier momento.
...
- **Resultado esperado**: Se limpia el borrador, se reinicia el asistente y aparece el diálogo de verificación de DNI.

#### Mensajes del Sistema:
- ⚠️ **Advertencia**: “Se canceló la postulación. Podés comenzar una nueva cuando quieras.”

#### Casos Especiales:
- Al cambiar el DNI desde el Paso 1, se aplica la misma lógica de reinicio que al cancelar.

### 4.9 Confirmación de Entrevistas (Portal de Seguimiento)
**Descripción**: Los postulantes con un enlace recibido por correo pueden confirmar o rechazar opciones de entrevista y consultar material adicional.
...
- **Validaciones**: Si el token es inválido o expiró, se muestra un error y no se cargan datos.
...
- **Validaciones**: Las opciones se deshabilitan mientras se envía la respuesta; si ya se registró una respuesta, se ocultan las acciones.
- **FOTO-ADJUNTA**: Botones con icono de calendario y texto “Elegir esta opción”.
- **Resultado esperado**: Se muestra una alerta de agradecimiento con la información confirmada o aviso de reprogramación.
...
- **Datos requeridos**: Ninguno.
- **Validaciones**: Los enlaces se abren en una pestaña nueva para su consulta.
...
#### Mensajes del Sistema:
- ✅ **Éxito**: Alertas dentro de la tarjeta con mensajes de agradecimiento y la fecha confirmada.
- ❌ **Error**: “El enlace es inválido o está incompleto.”; “No pudimos registrar tu respuesta. Intentá nuevamente.”; “No se pudo cargar la solicitud”.
- ⚠️ **Advertencia**: “Si necesitás modificar tu respuesta, comunicate con la escuela…” una vez registrada la asistencia.

#### Casos Especiales:
- Si el usuario indica que no puede asistir, el sistema registra una solicitud de reprogramación y la institución enviará nuevas fechas; el mensaje lo informa explícitamente.

## 5. Preguntas Frecuentes
1. **¿Puedo completar la postulación en varios días?** Sí, el sistema guarda automáticamente un borrador en el navegador y permite retomarlo más tarde.
2. **¿Qué pasa si el DNI del aspirante ya se postuló?** No podrá continuar; aparecerá el mensaje de DNI duplicado y deberá contactar a la escuela para continuar el proceso.
3. **¿Es obligatorio autorizar comunicaciones?** Sí, sin esa confirmación el botón “Enviar Postulación” permanece deshabilitado.
4. **¿Cómo confirmo la entrevista si pierdo el correo?** Solicite a la escuela un nuevo enlace; sin el token el portal de entrevistas no puede cargar la solicitud.

## 6. Solución de Problemas
- **No puedo pasar del Paso 1**: Verifique que todos los campos obligatorios estén completos, que el DNI tenga entre 7 y 10 dígitos y que la fecha de nacimiento sea válida.
- **El botón “Siguiente” sigue deshabilitado en el Paso 2**: Asegúrese de tener al menos un familiar completo y que ninguna tarjeta muestre campos resaltados en rojo.
- **Recibo un error al enviar la postulación**: Revise las fechas de nacimiento cargadas y la autorización de comunicaciones; el sistema detiene el envío si encuentra inconsistencias.
- **El portal de entrevistas indica enlace inválido**: Confirme que está usando el último correo enviado por la escuela; los enlaces caducados o incompletos generan ese mensaje y debe solicitar uno nuevo.
# Reportes

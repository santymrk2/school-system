...
const emisionesCuota = {
  list: () => http.get<DTO.EmisionCuotaDTO[]>("/api/emisiones-cuota"),
  byId: (id: number) =>
    http.get<DTO.EmisionCuotaDTO>("/api/emisiones-cuota/" + id),
  create: (body: DTO.EmisionCuotaCreateDTO) =>
    http.post<number>("/api/emisiones-cuota", body),
...
const cuotas = {
  ...
  update: (id: number, body: DTO.CuotaDTO) =>
    http.put<void>("/api/cuotas/" + id, body),
...
const pagosCuota = {
  ...
  updateEstado: (id: number, body: DTO.PagoCuotaEstadoUpdateDTO) =>
    http.patch<void>(`/api/pagos/${id}/estado`, body),
...
const recibos = {
  ...
  byId: (id: number) =>
    http.get<DTO.ReciboSueldoDTO>("/api/recibos-sueldo/" + id),
  create: (body: DTO.ReciboSueldoCreateDTO) =>
    http.post<number>("/api/recibos-sueldo", body),
  update: (id: number, body: DTO.ReciboSueldoDTO) =>
    http.put<void>("/api/recibos-sueldo/" + id, body),
...
export { emisionesCuota, cuotas, pagosCuota, recibos };
